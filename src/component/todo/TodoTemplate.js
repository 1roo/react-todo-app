...
const TodoTemplate = () => {
  ...
  // todoInput에 todoText를 받아오는 함수
  // 자식 컴포넌트가 부모 컴포넌트에게 데이터를 전달할 때는 props사용이 불가능.
  ...
  const addTodo = todoText => {
    ...
    //다만, 상태변수가 불변성(immutable)을 가지기 때문에 기존 상태에서 변경 불가하며,
  ...
  // useEffect(() => {
  //   console.log();
  // }, [todos]); //todos배열에 변화가 있다면 재렌더링을 해라.
