...
const Join = () => {
    ...
    const saveInputState = ({key, inputVal, flag, msg}) => {
        ...
        inputVal !== 'pass' && setUserValue({ //useState로 관리되는건 항상 setter로~
            //userValue들 중 다른 값들은 그대로 유지하고 userName만 inputVal로 교체. (구조 분해 할당)
            ...userValue,
            [key]: inputVal    
        });
        ...
        setMessage({
            ...message,
            // [] 안에 변수명을 주지 않으면 변수명을 새로운 프로퍼티로 해석해서 넣어버리기 때문에 
            // 변수 값으로 인식시키기 위해 [] 안에 변수명을 작성.
            [key]: msg
        });
    ...
    const nameHandler = e => {
        ...
        //입력한 값을 상태변수에 저장
        // console.log(e.target.value); //useState로 관리되는건 항상 setter로~
        ...
        if(!inputVal) {
            msg = '유저 이름은 필수입니다.';
        } else if(!nameRegex.test(inputVal)) { //inputVal이 nameRegex(정규표현식)랑 일치하면 true, 불일치면 false
